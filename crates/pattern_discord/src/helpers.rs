use serenity::{
    all::CreateMessage,
    client::Context,
    model::{channel::Message, id::ChannelId, mention::Mentionable, permissions::Permissions},
};
        ...
        Ok(channel) => {
            match channel {
                serenity::model::channel::Channel::Guild(guild_channel) => {
                    info!(
                        "Channel is a guild channel: {} in guild {}",
                        guild_channel.name, guild_channel.guild_id
                    );
                        ...
                        Ok(guild) => {
                            ...
                            match guild.member(&ctx.http, current_user).await {
                                Ok(member) => {
                                    ...
                                    let permissions =
                                        guild.user_permissions_in(&guild_channel, &member);
...
pub async fn send_with_fallback(ctx: &Context, msg: &Message, content: &str) -> Result<(), String> {
        ...
        if let serenity::model::channel::Channel::Guild(guild_channel) = channel {
            ...
            if let Ok(guild) = ctx.http.get_guild(guild_channel.guild_id).await {
                ...
                if let Ok(member) = guild.member(&ctx.http, current_user).await {
                    ...
                    info!(
                        "Bot member nick: {:?}, joined_at: {:?}",
                        member.nick, member.joined_at
                    );
                    ...
                    info!(
                        ...
                        guild_channel.name, channel_id, permissions
                    );
                    ...
                    if !permissions.contains(Permissions::READ_MESSAGE_HISTORY) {
                        info!(
                            "Missing READ_MESSAGE_HISTORY permission in channel {}",
                            channel_id
                        );
                    }
